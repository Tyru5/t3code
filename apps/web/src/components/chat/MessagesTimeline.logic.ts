  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    nextRows.push({
      ...
      showAssistantCopyButton:
        timelineEntry.message.role === "assistant" &&
        !input.timelineEntries.slice(index + 1).some((nextEntry) => {
          if (nextEntry?.kind !== "message" || nextEntry.message.role !== "assistant") {
            return false;
          }
          return nextEntry.message.turnId === timelineEntry.message.turnId;
        }),
    });
  }

  if (input.skillsCatalogCard) {
    nextRows.push({
      kind: "skills-catalog",
      id: input.skillsCatalogCard.id,
      createdAt: input.skillsCatalogCard.createdAt,
      card: input.skillsCatalogCard,
    });
  }
...
export function resolveAssistantMessageCopyState(input: {
  ...
  return {
    text: trimmedText.length > 0 ? input.text : null,
    visible: input.showCopyButton && !input.streaming && trimmedText.length > 0,
  };
}
