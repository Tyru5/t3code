...
const baseSkillsCatalog: SkillCatalogResult = {
  entries: [
    {
      id: "review",
      name: "review",
...
describe("wsNativeApi", () => {
  ...
  it("forwards skills catalog fetches directly to the RPC client", async () => {
    rpcClientMock.server.getSkillsCatalog.mockResolvedValue(baseSkillsCatalog);
    const { createWsNativeApi } = await import("./wsNativeApi");

    const api = createWsNativeApi();

    await expect(api.server.getSkillsCatalog()).resolves.toEqual(baseSkillsCatalog);
    expect(rpcClientMock.server.getSkillsCatalog).toHaveBeenCalledWith();
  });
